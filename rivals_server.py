...
def check_win():
    ...
    if mode in ('tdm', '1v1'):
        ...
        pass  # handled via koth_tick
    elif mode == 'ctf':
        if lobby['scores'].get('red', 0) >= t:    end_round('RED TEAM', 'red')
        elif lobby['scores'].get('blue', 0) >= t: end_round('BLUE TEAM', 'blue')
    elif mode == 'gun':
        for p in lobby['players'].values():
            if p.get('gunId') == 'sniper' and p['kills'] >= 1:
                end_round(p['name'] + ' WINS', 'ffa')
                break
    ...
    with lock:
        ...
        if t == 'set_name':
            ...
            if 'mode' in cfg:
                ...
                targets = {'tdm':30,'ffa':8,'koth':20,'ctf':3,'gun':None,'1v1':5}
            ...
            if pid != lobby['host_id']: return
            if lobby['phase'] != 'lobby': return
            # Need at least 1 player per team, or 1v1
            size = SIZE_MAP.get(lobby['config']['size'],2)
            reds  = [x for x in lobby['players'].values() if x['team']=='red']
            blues = [x for x in lobby['players'].values() if x['team']=='blue']
            # Fill with bots
            ...
            # For gun game: advance weapon on server so win check works
            if mode == 'gun':
                gun_order = ['pistol','smg','shotgun','assault','sniper']
                cur = p.get('gunId','pistol')
                idx = gun_order.index(cur) if cur in gun_order else 0
                if idx < len(gun_order)-1:
                    p['gunId'] = gun_order[idx+1]
...
function onServerMsg(msg){
  switch(msg.type){
    ...
    case 'start':
      ...
      hostId=msg.host_id;
      // Build remote player list from msg.players
      remotePlayers={};
    ...
    case 'state':
      ...
      if(remotePlayers[msg.id]){
        Object.assign(remotePlayers[msg.id], msg);
      } else {
        remotePlayers[msg.id]=msg;
      }
      break;
    case 'kill':
      handleKillMsg(msg);
...
function handleKillMsg(msg){
  ...
  // Update remote player kills
  if(msg.killer_id && remotePlayers[msg.killer_id]){
    remotePlayers[msg.killer_id].kills=msg.killer_kills||0;
    remotePlayers[msg.killer_id].streak=msg.streak||0;
  }

  // If I was the killer (confirmed by server) - update my local stats
  if(msg.killer_id===myId && localPlayer){
    localPlayer.kills=msg.killer_kills||localPlayer.kills;
    localPlayer.streak=msg.streak||localPlayer.streak;
    updStatsHud();
  ...
  // If I was the victim
  if(msg.victim_id===myId && localPlayer && !localPlayer.dead){
    localPlayer.hp=0;
    localPlayer.dead=true;
    localPlayer.deaths++;
    localPlayer.streak=0;
    localPlayer.swordHpFrac=0;
...
function renderLobby(msg){
  ...
  if(isHost){
    ...
    const size=parseInt(currentConfig.size)||2;
    const rReady=reds.filter(p=>p.ready).length;
    const bReady=blues.filter(p=>p.ready).length;
    const canStart=reds.length>=1&&blues.length>=1;
    document.getElementById('start-btn').disabled=!canStart;
    document.getElementById('status-msg').textContent=
      canStart?`RED ${rReady}/${reds.length} ready · BLUE ${bReady}/${blues.length} ready — Press START!`
      :`Need at least 1 player on each team (bots fill remaining)`;
  }
}
...
const SWORD_R=68,SWORD_ARC=Math.PI*.68,ATK_DUR=.25,SWORD_CD=0.9;
const SWORD_DMG=0.5; // takes 2 hits per HP
...
class Bullet{
  constructor(x,y,angle,shooterRef,gun,shooterId){
    ...
    this.shooterId=shooterId;this.shooterTeam=shooterRef.team;
  ...
  update(dt){
    ...
    // Hit local player
    if(!localPlayer.dead&&this.shooterId!==myId&&this.shooterTeam!==localPlayer.team){
      if(Math.hypot(localPlayer.x-this.x,localPlayer.y-this.y)<RAD+2){
        localPlayer.hp=Math.max(0,localPlayer.hp-this.dmg);
        localPlayer.hitFlash=.22;
        this.dead=true;
        ...
        if(localPlayer.hp<=0 && !localPlayer.dead){
          ...
          localPlayer.deaths++;
          localPlayer.streak=0;
          localPlayer.respawnTimer=RESPAWN_S;
          setRespawnUI(RESPAWN_S);
          spawnBurst(localPlayer.x,localPlayer.y,'#FF3131',28,true);
          wsSend({type:'kill',victim_id:myId,killer_id:this.shooterId});
        }
        return;
      }
    }
    // Hit bots (if I'm host)
    if(isHost && this.shooterId===myId){
      for(const bot of botEntities){
        if(bot.dead||bot.team===this.shooterTeam)continue;
        if(Math.hypot(bot.x-this.x,bot.y-this.y)<RAD+2){
          this.dead=true;
          spawnBurst(this.x,this.y,this.col,10);
          bot.hp=Math.max(0,bot.hp-this.dmg);
          if(bot.hp<=0&&!bot.dead){
            bot.dead=true;bot.deaths++;
            bot.respawnTimer=RESPAWN_S;
            spawnBurst(bot.x,bot.y,bot.teamCol,28,true);
            localPlayer.kills++;localPlayer.streak++;
            gameScores[localPlayer.team]=(gameScores[localPlayer.team]||0)+1;
            wsSend({type:'kill',victim_id:bot.id,killer_id:myId,streak:localPlayer.streak});
            if(STREAK_NAMES[localPlayer.streak])showStreak(STREAK_NAMES[localPlayer.streak]);
...
function makeLocalPlayer(info){
  return{
    ...
    respawnTimer:0,
    gunIndex:0,
    swordHpFrac:0, // fractional sword damage (2 hits = 1 HP)
...
class BotEntity{
  ...
  respawn(){
    ...
    this.x=x;this.y=y;this.hp=MAX_HP;this.dead=false;this.hitFlash=0;this.swordHpFrac=0;this.shield=0;
  ...
  update(dt){
    ...
    if(this._forcedWander>0){
      ...
      if(!target){
        ...
        // Melee vs local player
        if(localPlayer&&!localPlayer.dead&&this.team!==localPlayer.team&&this.atkCD<=0){
          const md=Math.hypot(localPlayer.x-this.x,localPlayer.y-this.y);
          if(md<SWORD_R+RAD){
            // Start melee
            this.attacking=true;this.atkTimer=ATK_DUR;this.atkCD=SWORD_CD;
            // Apply sword damage to player (shield first)
            const lp=localPlayer;
            if(lp.shield>0){
              lp.shield=Math.max(0,lp.shield-1);lp.hitFlash=.2;
              spawnBurst(lp.x,lp.y,'#00F5FF',6);updHpHud();
            } else {
              lp.swordHpFrac=(lp.swordHpFrac||0)+SWORD_DMG;
              lp.hitFlash=.2;
              if(lp.swordHpFrac>=1){lp.swordHpFrac=0;lp.hp=Math.max(0,lp.hp-1);updHpHud();}
              if(lp.hp<=0&&!lp.dead){
                lp.dead=true;lp.deaths++;lp.streak=0;lp.respawnTimer=RESPAWN_S;
                setRespawnUI(RESPAWN_S);spawnBurst(lp.x,lp.y,'#FF3131',28,true);
                wsSend({type:'kill',victim_id:myId,killer_id:this.id});
              }
            }
            shake(4);
          }
    ...
    // Broadcast bot state
    wsSend({type:'state',id:this.id,x:this.x,y:this.y,angle:this.angle,hp:this.hp,dead:this.dead,vx:this.vx,vy:this.vy,gunId:this.gun.id,ammo:this.ammo,reloading:this.reloading,hasFlag:false,attacking:this.attacking,skin:this.skin.id,name:this.name,team:this.team});
  }
  ...
  draw(){
    ...
    c.strokeStyle='rgba(210,215,230,.8)';c.lineWidth=2;c.stroke();
    c.beginPath();c.moveTo(RAD+9,-7);c.lineTo(RAD+9,7);
    c.strokeStyle='rgba(190,195,220,.7)';c.lineWidth=2.5;c.stroke();
    c.restore();
...
function drawRemotePlayer(p){
  ...
  c.beginPath();c.moveTo(RAD-1,-1.5);c.lineTo(SWORD_R*.85,0);c.strokeStyle='rgba(210,215,230,.8)';c.lineWidth=2;c.stroke();
  c.restore();
  c.restore();
...
function drawLocalPlayer(){
  ...
  c.save();c.rotate(sa);
  c.shadowColor=p.attacking?'rgba(255,230,0,.85)':'transparent';c.shadowBlur=p.attacking?12:0;
  c.beginPath();c.moveTo(RAD-1,-1.5);c.lineTo(SWORD_R*.85,0);c.strokeStyle=p.attacking?'#FFE600':'rgba(210,215,230,.8)';c.lineWidth=p.attacking?3:2;c.stroke();
  c.beginPath();c.moveTo(RAD+9,-7);c.lineTo(RAD+9,7);c.strokeStyle=p.attacking?'rgba(255,230,0,.9)':'rgba(190,195,220,.7)';c.lineWidth=2.5;c.stroke();
...
function loop(ts){
  ...
  if(isHost) botEntities.forEach(b=>b.update(dt));
...
function checkMeleeHits(){
  ...
  botEntities.forEach(b=>{
    ...
    if(Math.abs(diff)<=SWORD_ARC){
      ...
      spawnBurst(b.x,b.y,'rgba(255,220,0,.8)',4);
      // Shield absorbs sword hit first
      if((b.shield||0)>0){
        b.shield--;b.hitFlash=.2;
        spawnBurst(b.x,b.y,'#00F5FF',8);
      } else {
        // Half damage: 2 hits = 1 HP
        b.swordHpFrac=(b.swordHpFrac||0)+SWORD_DMG;
        b.hitFlash=.2;
        if(b.swordHpFrac>=1){b.swordHpFrac=0;b.hp=Math.max(0,b.hp-1);}
        if(b.hp<=0&&!b.dead){
...
function respawnLocal(){
  const p=localPlayer;
  p.hp=MAX_HP;p.dead=false;p.hitFlash=0;p.shield=0;p.swordHpFrac=0;
...
function doShoot(){
  const p=localPlayer;
  if(p.fireCooldown>0||p.reloading||p.ammo<=0||p.dead)return;
...
function render(){
  ...
  Object.values(remotePlayers).forEach(p=>{if(p&&!p.bot)drawRemotePlayer(p);});
  drawLocalPlayer();
  ctx.restore();
}
...
function updScoreHud(){
  ...
  if(m.mode==='tdm'||m.mode==='koth'||m.mode==='ctf'||m.mode==='1v1'){
  ...
  } else if(m.mode==='ffa'||m.mode==='gun'||m.mode==='1v1'){
    const myK=localPlayer?localPlayer.kills:0;
    // Find highest kill count among all
    const allKills=[myK,...Object.values(remotePlayers).map(r=>r.kills||0),...botEntities.map(b=>b.kills||0)];
    const topK=Math.max(...allKills);
    ...
    document.getElementById('ssep').innerHTML=m.mode==='gun'?'WEAPON<br>TIER':'MY KILLS<br>TO '+(t||'?');
  }
  // Big score pulse
