...
/* ── LOBBY PANEL (shown on CODE tab when in a room) ── */
#lobby-panel{position:fixed;bottom:1rem;right:1rem;width:320px;z-index:200;display:none;flex-direction:column;gap:.5rem}
#lobby-panel.show{display:flex}
#lobby-players{background:rgba(5,5,8,.96);border:2px solid var(--bdr);padding:.6rem}
#lobby-players-title{font-family:'Press Start 2P',monospace;font-size:.26rem;color:#5a5a90;letter-spacing:.1em;margin-bottom:.5rem}
.lp-card{display:flex;align-items:center;gap:.5rem;padding:.35rem .4rem;border:1px solid var(--bdr);margin-bottom:.3rem;background:rgba(255,255,255,.02)}
.lp-card:last-child{margin-bottom:0}
.lp-emoji{font-size:1.3rem;line-height:1}
.lp-info{flex:1;min-width:0}
.lp-name{font-family:'Press Start 2P',monospace;font-size:.26rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.lp-sub{font-family:'Press Start 2P',monospace;font-size:.2rem;color:#5a5a90;margin-top:.15rem}
.lp-badge{font-family:'Press Start 2P',monospace;font-size:.2rem;padding:.1rem .3rem;border:1px solid}
#lobby-chat{background:rgba(5,5,8,.96);border:2px solid var(--bdr);display:flex;flex-direction:column}
#lobby-chat-msgs{height:200px;overflow-y:auto;padding:.5rem .6rem;display:flex;flex-direction:column;gap:.3rem}
#lobby-chat-msgs::-webkit-scrollbar{width:3px}
#lobby-chat-msgs::-webkit-scrollbar-thumb{background:#2a2a48}
.lc-msg{font-size:.75rem;line-height:1.4;word-break:break-word}
.lc-msg .lc-who{font-family:'Press Start 2P',monospace;font-size:.22rem;margin-right:.3rem}
.lc-msg .lc-who.me{color:var(--y)}.lc-msg .lc-who.them{color:var(--c)}.lc-msg .lc-who.skin{color:#a78bfa}
#lobby-chat-input-row{display:flex;border-top:1px solid var(--bdr)}
#lobby-chat-input{flex:1;background:transparent;border:none;color:#f0f0ff;font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:1rem;padding:.4rem .55rem;outline:none}
#lobby-chat-input::placeholder{color:#2a2a48;font-size:.8rem}
#lobby-chat-send{font-family:'Press Start 2P',monospace;font-size:.22rem;background:transparent;border:none;border-left:1px solid var(--bdr);color:var(--y);padding:.35rem .5rem;cursor:pointer}
#lobby-chat-send:hover{background:rgba(255,230,0,.07)}
 
/* ── WIN SCREEN OVERLAY ── */
#win-screen{position:fixed;inset:0;z-index:200;display:none;flex-direction:column;align-items:center;justify-content:center;background:rgba(5,5,8,.97);backdrop-filter:blur(20px)}
#win-screen.show{display:flex}
#win-crown{font-size:clamp(3rem,10vw,6rem);animation:crownpop .6s cubic-bezier(.17,.67,.35,1.4) both}
@keyframes crownpop{from{opacity:0;transform:scale(.2) rotate(-20deg)}to{opacity:1;transform:scale(1) rotate(0)}}
#win-label{font-family:'Press Start 2P',monospace;font-size:clamp(1.2rem,4vw,3rem);margin:.4rem 0 .2rem;text-align:center;animation:wilfadein .5s .3s both}
#win-sub{font-family:'Press Start 2P',monospace;font-size:clamp(.3rem,1.2vw,.6rem);color:#5a5a90;letter-spacing:.15em;margin-bottom:.3rem;text-align:center;animation:wilfadein .5s .45s both}
#win-kills{font-family:'Press Start 2P',monospace;font-size:clamp(.5rem,2vw,1rem);margin-bottom:2rem;text-align:center;animation:wilfadein .5s .55s both}
#win-btns{display:flex;gap:1rem;animation:wilfadein .5s .7s both}
@keyframes wilfadein{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:none}}
.win-btn{font-family:'Press Start 2P',monospace;font-size:clamp(.3rem,1.2vw,.5rem);padding:.8rem 1.8rem;border:2px solid;background:transparent;cursor:pointer;transition:all .18s;letter-spacing:.06em}
.win-btn.primary{border-color:var(--y);color:var(--y)}.win-btn.primary:hover{background:var(--y);color:#000;box-shadow:0 0 24px rgba(255,230,0,.5)}
.win-btn.secondary{border-color:#3a3a60;color:#5a5a90}.win-btn.secondary:hover{border-color:#888;color:#ccc}
#win-particles{position:absolute;inset:0;pointer-events:none;overflow:hidden}
...
/* Left joystick */
#joy-left-zone{position:absolute;left:0;bottom:0;width:45%;height:55%;pointer-events:all}
#joy-left-base{position:absolute;width:110px;height:110px;border-radius:50%;background:rgba(255,255,255,.08);border:2px solid rgba(255,255,255,.18);transform:translate(-50%,-50%);display:none}
#joy-left-stick{position:absolute;width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,.35);border:2px solid rgba(255,255,255,.6);transform:translate(-50%,-50%);display:none}
 
/* Right joystick (aim + auto-shoot) */
#joy-right-zone{position:absolute;right:0;bottom:0;width:45%;height:55%;pointer-events:all}
#joy-right-base{position:absolute;width:110px;height:110px;border-radius:50%;background:rgba(255,49,49,.08);border:2px solid rgba(255,49,49,.25);transform:translate(-50%,-50%);display:none}
#joy-right-stick{position:absolute;width:48px;height:48px;border-radius:50%;background:rgba(255,49,49,.45);border:2px solid rgba(255,80,80,.8);transform:translate(-50%,-50%);display:none}
 
/* Action buttons */
#mob-btns{position:absolute;right:12px;bottom:200px;display:flex;flex-direction:column;gap:10px;pointer-events:all}
.mob-btn{width:52px;height:52px;border-radius:50%;border:2px solid;background:rgba(5,5,8,.85);font-family:'Press Start 2P',monospace;font-size:.32rem;display:flex;align-items:center;justify-content:center;cursor:pointer;-webkit-tap-highlight-color:transparent;user-select:none;flex-direction:column;gap:2px;line-height:1.2;text-align:center}
#mob-reload{border-color:#FFE600;color:#FFE600}
#mob-shop{border-color:#00E5A0;color:#00E5A0}
#mob-sword{border-color:#a78bfa;color:#a78bfa}
 
/* Menu touch improvements */
.isMobile .wrap{padding:0 .5rem}
.isMobile input.field,.isMobile .auth-field{font-size:16px!important} /* prevent iOS zoom */
.isMobile .tbt{padding:.9rem}
.isMobile #play-btn{padding:1.1rem}
.isMobile .cbtn{padding:.9rem}
 
/* Guest locked state */
.guest-locked .mode-btn{opacity:.45;cursor:not-allowed;pointer-events:none}
.guest-locked .mode-btn.on{opacity:.8;border-color:var(--y)}
.guest-locked .tbt{opacity:.45;cursor:not-allowed;pointer-events:none}
.guest-locked .tbt.on{opacity:1}
#guest-lock-notice{display:none;font-family:'Press Start 2P',monospace;font-size:.24rem;color:#5a5a90;text-align:center;padding:.4rem;border:1px solid #2a2a48;margin-bottom:.5rem}
.guest-locked #guest-lock-notice{display:block}
 
/* ── CHEAT CODE BOX ── */
#cheat-wrap{margin-top:.6rem;display:flex;gap:.4rem;align-items:center}
#cheat-in{flex:1;background:rgba(0,0,0,.6);border:2px solid #1a1a30;color:#5a5a90;font-family:'Press Start 2P',monospace;font-size:.28rem;padding:.45rem .6rem;letter-spacing:.1em;outline:none;transition:border-color .2s}
#cheat-in:focus{border-color:#5a5a90;color:#c0c0e0}
#cheat-in.activated{border-color:#00E5A0!important;color:#00E5A0!important;animation:cheatpulse .5s ease}
#cheat-in.denied{border-color:#FF3131!important;color:#FF3131!important}
@keyframes cheatpulse{0%,100%{box-shadow:none}50%{box-shadow:0 0 16px rgba(0,229,160,.6)}}
#cheat-btn{font-family:'Press Start 2P',monospace;font-size:.24rem;background:transparent;border:2px solid #1a1a30;color:#3a3a60;padding:.45rem .6rem;cursor:pointer;transition:all .18s;white-space:nowrap}
#cheat-btn:hover{border-color:#5a5a90;color:#aaa}
#cheat-msg{font-family:'Press Start 2P',monospace;font-size:.22rem;min-height:1em;margin-top:.2rem;text-align:center}
</style>
</head>
<body>
<div id="flash"></div>
  ...
  <div id="auth-box">
    ...
    <div class="auth-card">
      ...
      <div class="auth-field-wrap">
        ...
        <input id="auth-pass" class="auth-field" type="password" placeholder="••••••••" autocomplete="current-password"/>
      </div>
      <div class="auth-field-wrap" id="auth-pass2-wrap" style="display:none">
        <span class="auth-lbl">CONFIRM PASSWORD</span>
        <input id="auth-pass2" class="auth-field" type="password" placeholder="••••••••" autocomplete="new-password"/>
      </div>
...
<div class="wrap">
  <div id="profile-bar">
    ...
    <button id="profile-logout" onclick="authLogout()">SIGN IN / SIGN UP</button>
  </div>
  <h1>RIVALS</h1>
  ...
  <div class="row2">
    ...
    <div class="card">
      ...
      <div class="team-row">
        <button class="tbt red on" id="tbr" onclick="pickTeam('red')">🔴 RED</button>
        <button class="tbt blue" id="tbb" onclick="pickTeam('blue')">🔵 BLUE</button>
      </div>
    </div>
  </div>
  <!-- TAB BAR -->
  <div id="menu-tabs">
    <button class="mtab on" id="mtab-play"      onclick="switchTab('play')"><span class="ti">⚔️</span>PLAY</button>
    <button class="mtab"    id="mtab-inventory" onclick="switchTab('inventory')"><span class="ti">🎒</span>INVENTORY</button>
    <button class="mtab"    id="mtab-shop"      onclick="switchTab('shop')"><span class="ti">🛒</span>SHOP</button>
    <button class="mtab"    id="mtab-code"      onclick="switchTab('code')"><span class="ti">🔗</span>CODE</button>
  </div>
  <div class="tab-body">
    <!-- PLAY TAB -->
    <div id="tp-play" class="tp on">
      <span class="lbl">SELECT MODE</span>
      <div id="guest-lock-notice">🔒 HOST CONTROLS MODE &amp; TEAM</div><div class="mode-list" id="mode-list"></div>
      <button id="play-btn" onclick="startSolo()">⚡ PLAY NOW</button>
      <div class="hints">
      ...
      <div id="cheat-wrap">
        <input id="cheat-in" type="text" placeholder="CHEAT CODE" maxlength="16" autocomplete="off" spellcheck="false" onkeydown="if(event.key==='Enter')submitCheat()"/>
        <button id="cheat-btn" onclick="submitCheat()">ENTER</button>
      </div>
      <div id="cheat-msg"></div>
    </div>
    <!-- INVENTORY TAB -->
    <div id="tp-inventory" class="tp">
      <span class="lbl" style="margin-bottom:.5rem">YOUR WEAPONS</span>
      <p style="font-family:'Press Start 2P',monospace;font-size:.22rem;color:#5a5a90;margin-bottom:.6rem;line-height:2">ONE GUN EQUIPPED AT A TIME · CARRIES INTO EVERY MATCH</p>
      <div class="gun-cards" id="inv-gun-cards" style="margin-bottom:1.2rem"></div>
    ...
    <div id="tp-shop" class="tp">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem">
        <span class="lbl" style="margin-bottom:0">WEAPON SHOP</span>
        <span id="menu-coins-disp" style="font-family:'Press Start 2P',monospace;font-size:.3rem;color:#00E5A0">💰 0 COINS</span>
      </div>
      <p style="font-family:'Press Start 2P',monospace;font-size:.24rem;color:#5a5a90;margin-bottom:.7rem;line-height:2">BUY GUNS HERE OR IN-GAME WITH [B] · COINS SAVE FOREVER</p>
    ...
    <div id="tp-code" class="tp">
      <div class="code-wrap">
        ...
        <div class="code-btns">
          <button class="cbtn make" onclick="makeRoom()">🌐 CREATE ROOM</button>
          <button class="cbtn join" onclick="joinRoom()">🔗 JOIN ROOM</button>
        </div>
        <div id="room-code-big"></div>
        <div id="room-status"></div>
        <button id="lobby-start-btn" onclick="hostStartGame()" style="display:none;width:100%;font-family:'Press Start 2P',monospace;font-size:.5rem;letter-spacing:.1em;background:var(--y);color:#000;border:none;cursor:pointer;padding:.9rem;margin-top:.4rem;transition:transform .15s,filter .15s">▶ START GAME</button>
        <div class="code-steps">1️⃣ CREATE ROOM → SHARE 4-DIGIT CODE<br>2️⃣ FRIEND ENTERS CODE → CLICKS JOIN<br>3️⃣ GAME STARTS AUTOMATICALLY ✓</div>
      </div>
    </div>
  </div>
</div>
...
<!-- LOBBY PANEL (fixed bottom-right, visible on CODE tab when in a room) -->
<div id="lobby-panel">
  <div id="lobby-players">
    <div id="lobby-players-title">👥 PLAYERS IN ROOM</div>
    <div id="lobby-player-list"></div>
  </div>
  <div id="lobby-chat">
    <div id="lobby-chat-msgs"></div>
    <div id="lobby-chat-input-row">
      <input id="lobby-chat-input" placeholder="say something…" maxlength="60" autocomplete="off" spellcheck="false"/>
      <button id="lobby-chat-send" onclick="lobbyChatSend()">SEND</button>
    </div>
  </div>
</div>
...
<div id="game" class="screen">
  ...
  <div id="mobile-controls">
    ...
    <div id="joy-left-zone">
      <div id="joy-left-base"><div id="joy-left-stick"></div></div>
    </div>
    <!-- Right joystick -->
    <div id="joy-right-zone">
      <div id="joy-right-base"><div id="joy-right-stick"></div></div>
    </div>
    <!-- Action buttons -->
    <div id="mob-btns">
      <div class="mob-btn" id="mob-reload">🔄<span>RELOAD</span></div>
      <div class="mob-btn" id="mob-shop">🛒<span>SHOP</span></div>
      <div class="mob-btn" id="mob-sword">⚔️<span>ATK</span></div>
    </div>
  </div>
  ...
  <div id="hud">
    ...
    <div id="coins-hud">💰 <span id="coin-count">0</span></div>
    <div id="feed"></div>
    <div id="bot-hud">
      <div id="hp-bar"><span class="hl">HP</span><span class="hrt" id="h1">❤️</span><span class="hrt" id="h2">❤️</span><span class="hrt" id="h3">❤️</span><span class="hrt" id="h4" style="display:none">❤️</span><span class="hrt" id="h5" style="display:none">❤️</span><span id="shld"></span></div>
      <div id="pow-hud"></div>
      <div id="ammo-bar"><span id="gun-lbl">PISTOL</span><div id="bullets"></div><span id="reload-txt">RELOADING…</span></div>
    </div>
    <div id="map-lbl"></div>
    <div id="respawn-screen"><div id="rs-label">— ELIMINATED —</div><div id="rs-count">3</div></div>
    ...
    <div id="duel-hud" style="display:none;position:absolute;top:.5rem;left:50%;transform:translateX(-50%);font-family:'Press Start 2P',monospace;font-size:.3rem;color:#FFE600;background:rgba(5,5,8,.9);border:1px solid rgba(255,230,0,.3);padding:.3rem .8rem;white-space:nowrap;letter-spacing:.05em;z-index:20;text-align:center">
      <div id="duel-score">YOU 0 — 0 BOT  |  ROUND 1/10</div>
    </div>
  </div>
...
<div id="shop-overlay">
  ...
  <div id="shop-coins-disp">💰 0 COINS</div>
  <div style="display:flex;gap:.5rem;margin-bottom:.8rem">
    <button id="stab-guns" class="shop-tab-btn on" onclick="switchShopTab('guns')">🔫 GUNS</button>
    <button id="stab-items" class="shop-tab-btn" onclick="switchShopTab('items')">⚡ ITEMS</button>
  </div>
  ...
  <div id="shop-msg"></div>
  <button id="shop-close" onclick="closeShop()">✕ CLOSE [B]</button>
</div>
...
<div id="win-screen">
  ...
  <div id="win-label">YOU WIN!</div>
  <div id="win-sub">FIRST TO 10 KILLS</div>
  <div id="win-kills"></div>
  <div id="win-btns">
    <button class="win-btn primary" onclick="winPlayAgain()">⚡ PLAY AGAIN</button>
    <button class="win-btn secondary" onclick="winGoMenu()">🏠 MENU</button>
  </div>
</div>
...
<div id="end" class="screen">
  ...
  <table><thead><tr><th>#</th><th>PLAYER</th><th>TEAM</th><th>K</th><th>D</th><th>K/D</th><th>SCORE</th><th>💰</th></tr></thead><tbody id="sb"></tbody></table>
  <button id="again-btn" onclick="endScreenReturn()">PLAY AGAIN ⚡</button>
</div>
...
<div id="pause" style="display:none;position:fixed;inset:0;background:rgba(5,5,8,.92);backdrop-filter:blur(16px);z-index:100;flex-direction:column;align-items:center;justify-content:center">
  ...
  <div style="display:flex;flex-direction:column;gap:.7rem;width:100%;max-width:270px">
    <button onclick="togglePause()" style="font-family:'Press Start 2P',monospace;font-size:.42rem;background:transparent;border:2px solid #FFE600;color:#FFE600;padding:.8rem;cursor:pointer" onmouseover="this.style.background='rgba(255,230,0,.12)'" onmouseout="this.style.background='transparent'">▶ RESUME</button>
    <button onclick="openShop()" style="font-family:'Press Start 2P',monospace;font-size:.42rem;background:transparent;border:2px solid #00E5A0;color:#00E5A0;padding:.8rem;cursor:pointer" onmouseover="this.style.background='rgba(0,229,160,.1)'" onmouseout="this.style.background='transparent'">🛒 GUN SHOP</button>
    <button onclick="exitToMenu()" style="font-family:'Press Start 2P',monospace;font-size:.42rem;background:transparent;border:2px solid #FF3131;color:#FF3131;padding:.8rem;cursor:pointer" onmouseover="this.style.background='rgba(255,49,49,.12)'" onmouseout="this.style.background='transparent'">✕ EXIT TO MENU</button>
  </div>
</div>
...
// ═══════════════════════════════════
const MODES=[
  {id:'tdm', icon:'⚔️',  name:'TEAM DM',    desc:'First to 30 kills',target:30,teams:true},
  {id:'1v1', icon:'🥊',  name:'1V1',        desc:'Best of 10 rounds vs bot',target:6,teams:false,duel:true},
  {id:'ffa', icon:'💀',  name:'FREE 4 ALL', desc:'First to 8 kills', target:8, teams:false},
  {id:'koth',icon:'👑',  name:'KING OF HILL',desc:'Hold zone 20s',   target:20,teams:true},
  {id:'ctf', icon:'🚩',  name:'CAPTURE FLAG',desc:'Capture 3 flags', target:3, teams:true},
  {id:'gun', icon:'🔫',  name:'GUN GAME',   desc:'Cycle all weapons',target:null,teams:false},
  {id:'ccs', icon:'🏛️',  name:'CONTROL',     desc:'Cap 3 zones · First to 100pts',target:100,teams:true},
];
const SKINS=[
  {id:'phantom', col:'#8B5CF6',glow:'rgba(139,92,246,.8)', emoji:'👻',name:'PHANTOM', shape:'circle',  accent:'#C4B5FD'},
  {id:'demon',   col:'#DC2626',glow:'rgba(220,38,38,.8)',  emoji:'😈',name:'DEMON',   shape:'diamond',  accent:'#FCA5A5'},
  {id:'ghost',   col:'#94A3B8',glow:'rgba(200,210,230,.6)',emoji:'🤍',name:'GHOST',   shape:'circle',   accent:'#E2E8F0'},
  {id:'blaze',   col:'#F97316',glow:'rgba(249,115,22,.8)', emoji:'🔥',name:'BLAZE',   shape:'triangle', accent:'#FED7AA'},
  {id:'void',    col:'#4F46E5',glow:'rgba(99,102,241,.8)', emoji:'⚫',name:'VOID',    shape:'square',   accent:'#A5B4FC'},
  {id:'storm',   col:'#0891B2',glow:'rgba(8,145,178,.8)',  emoji:'⚡',name:'STORM',   shape:'circle',   accent:'#67E8F9'},
  {id:'neon',    col:'#00E5A0',glow:'rgba(0,229,160,.8)',  emoji:'💚',name:'NEON',    shape:'diamond',  accent:'#6EE7B7'},
  {id:'crimson', col:'#FF0080',glow:'rgba(255,0,128,.8)',  emoji:'🩸',name:'CRIMSON', shape:'triangle', accent:'#FDB4D8'},
  {id:'frost',   col:'#7DD3FC',glow:'rgba(125,211,252,.8)',emoji:'❄️',name:'FROST',   shape:'square',   accent:'#BAE6FD'},
  {id:'gold',    col:'#EAB308',glow:'rgba(234,179,8,.8)',  emoji:'👑',name:'GOLD',    shape:'circle',   accent:'#FDE68A'},
  {id:'shadow',  col:'#1E1E35',glow:'rgba(80,80,160,.8)',  emoji:'🖤',name:'SHADOW',  shape:'square',   accent:'#6366F1'},
  {id:'toxic',   col:'#84CC16',glow:'rgba(132,204,22,.8)', emoji:'☣️',name:'TOXIC',   shape:'triangle', accent:'#D9F99D'},
];
// All guns balanced to 12 skill points each
// Points: dmg(×3), rate(fire speed ×2), ammo(×1), reload(speed ×1), range(×1), mobility(×1), pierce(×3)
const GUNS=[
  ...
  // CHEAT GUN — unlocked by code ven7709 only, 50 skill points
  {id:'godgun', name:'GOD GUN', col:'#FFE600',dmg:8,  rate:.08, spd:1400,spread:.005,ammo:99,reload:.3, pel:5,range:3.0,tw:4,   price:-1,  desc:'⚡ CHEAT CODE UNLOCKED. 50 skill points.', pts:'CHEAT'},
];
...
// Picks a random non-cheat gun for a bot. Excludes 'godgun' (GUN_ORDER already
// has no godgun in it) and, when excludeId is given, avoids handing the bot
// the exact same gun the player currently has equipped.
function randomBotGun(excludeId){
  const pool = excludeId ? GUN_ORDER.filter(id=>id!==excludeId) : GUN_ORDER;
  const choices = pool.length ? pool : GUN_ORDER; // fallback if pool empties out
  return GUN_MAP[choices[Math.floor(Math.random()*choices.length)]];
}
const POWS=[
  {id:'health',icon:'❤️',col:'#FF3131',lbl:'HEALTH +1', price:10},
  {id:'speed', icon:'⚡',col:'#FFE600',lbl:'SPEED BOOST',price:15},
  {id:'damage',icon:'🔥',col:'#F97316',lbl:'DAMAGE AMP', price:20},
  {id:'shield',icon:'🛡️',col:'#00F5FF',lbl:'SHIELD',     price:25},
  {id:'ammo',  icon:'🔋',col:'#A3E635',lbl:'FULL RELOAD', price:10},
];
...
let equippedGunId='pistol'; // which gun player starts each life with
let playerMaxHp=3; // can be raised by cheat code (max 5)
// 1v1 round system
let duelRounds={player:0,bot:0},duelRound=0,duelBot=null,duelBetween=false;
 
// ── Multiplayer remote players ──
let myPid=null;
let remotePlayers={}; // pid → {x,y,angle,name,team,skin,hp,dead,gun,kills,deaths,score,coins}
...
class Entity{
  ...
  die(src){
    ...
    this.respT=RSP;
    burst(this.x,this.y,this.tc,14,true);burst(this.x,this.y,'#FFE600',5,true);
    if(src){
      src.kills++;src.streak++;src.score+=100;
      src.coins+=5; // earn coins per kill
      if(src.isP){_savePersistent();_guestSave();} // save immediately on every kill
    ...
    // 1v1 round check — bot or human peer killed us
    if(mode.id==='1v1'&&!gameOver&&!duelBetween&&performance.now()-gameStartT>2000){
      if(this.isP){
        // I died — bot/peer wins the round (only if peer not already counting it via duelRoundWon)
        duelRounds.bot++;endDuelRound('BOT');
      } else if(this===duelBot){
        // Bot died (no human peer) — I win
  ...
  respawn(){
    ...
    this.x=x;this.y=y;this.hp=this.isP?playerMaxHp:MAX_HP;this.dead=false;this.hitF=0;
    this.shield=0;this.swordHits=0;this.reloading=false;this.reloadT=0;this.fireCD=0;this.atkCD=0;
    // Reset to equipped gun on respawn (not in gun game) but KEEP owned guns
    if(mode.id!=='gun'){
      this.setGun(GUN_MAP[equippedGunId]||GUN_MAP['pistol']);
      // Do NOT wipe ownedGuns — player keeps purchased guns between lives
    }else{
  ...
  shoot(a){
    ...
    if(isMultiplayer&&this.isP)peerSend({type:'shoot',pid:myPidServer,x:this.x,y:this.y,angle:a,gun:this.gun.id});
  }
  ...
  upPlayer(){
    let dx=0,dy=0;
    if(isMobile && this.isP && (this._mobDx||this._mobDy)){
      dx=this._mobDx||0; dy=this._mobDy||0;
    } else {
    ...
    const shouldShoot = mdown || (isMobile && this.isP && joyR.active && Math.hypot(joyR.dx,joyR.dy)>.3);
    if(shouldShoot&&!shopOpen)this.shoot(this.angle);
    // Send position to friend
    if(isMultiplayer){
      this._stateT=(this._stateT||0)+1;
      if(this._stateT>=3){this._stateT=0; // send every 3rd frame (~20/s)
        peerSend({type:'state',pid:myPidServer,x:this.x,y:this.y,angle:this.angle,vx:this.vx,vy:this.vy});}
    }
  }
  ...
  upBot(dt){
    ...
    // Target — always chase player if reachable
  ...
  draw(){
    ...
    c.shadowColor=this.isP?this.skin.glow:'transparent';c.shadowBlur=this.isP?18:0;
    ...
    if(this.shield>0){
      ...
      c.strokeStyle='rgba(0,245,255,.7)';c.lineWidth=2.5;c.stroke();
    }
    ...
    const tag=this.isP?'▶ YOU':this.name+(this.isP?'':' 🤖');
...
const MAX_PARTS=120;
function burst(x,y,col,n,big=false){
  const add=Math.min(n, MAX_PARTS-particles.length);
  for(let i=0;i<add;i++)particles.push(new Particle(x,y,col,big));
}
 
class Powerup{
  constructor(x,y,t){this.x=x;this.y=y;this.type=t;this.anim=Math.random()*Math.PI*2;this.dead=false;}
  draw(){
    const bob=Math.sin(this.anim)*4;this.anim+=.03;
...
function checkWin(){
  ...
  const t=mode.target;
  if(mode.id==='1v1'){
    // Round ends when either player or bot dies — handled in die()
    return;
  }else if(mode.id==='tdm'){
...
function _endGameCore(txt,cls){
  ...
  setTimeout(()=>{
    document.getElementById('end-title').textContent=(won?'🏆 VICTORY! ':'')+txt;
    document.getElementById('end-title').className=cls;
    let endSubTxt=mode.id==='1v1'?'1V1 DUEL — YOU '+duelRounds.player+' ROUNDS — BOT '+duelRounds.bot+' ROUNDS':mode.id==='ccs'?'CONTROL — RED '+Math.floor(ccsScore.red)+' — BLUE '+Math.floor(ccsScore.blue)+' PTS':mode.name+' — ROUND OVER';
    ...
    const ab=document.getElementById('again-btn');
    if(ab)ab.textContent=isMultiplayer?'◀ BACK TO LOBBY':'PLAY AGAIN ⚡';
    showScreen('end');
    // In multiplayer auto-return to lobby after 8 seconds
    if(isMultiplayer){
      let secs=8;
      const ab2=document.getElementById('again-btn');
      const tick=setInterval(()=>{
        secs--;
        if(ab2)ab2.textContent='◀ BACK TO LOBBY ('+secs+'s)';
        if(secs<=0){clearInterval(tick);endScreenReturn();}
      },1000);
    }
  },900);
}
function buildSB(){
  ...
  if(mode.id==='1v1'){
    ...
    const rows=[
      ...
      {name:'RIVAL 🤖',rounds:duelRounds.bot,kills:duelBot?duelBot.kills:0,deaths:duelBot?duelBot.deaths:0,coins:0,me:false}
    ].sort((a,b)=>b.rounds-a.rounds);
    rows.forEach((r,i)=>{
      const tr=document.createElement('tr');
      if(r.me)tr.className='me';
      tr.innerHTML=`<td>${i===0?'👑':'#2'}</td><td>${r.me?'<b>▶ '+r.name+'</b>':r.name}</td><td><span style="color:var(--y)">ROUNDS</span></td><td class="tk">${r.rounds}</td><td class="tkd">${r.kills}</td><td>${r.deaths}</td><td class="ts">${(r.rounds*100).toLocaleString()}</td><td class="co">${r.coins}</td>`;
      tb.appendChild(tr);
    });
    // Update header for 1v1
    document.querySelector('#sb').closest('table').querySelector('thead tr').innerHTML=
      '<th>#</th><th>PLAYER</th><th>MODE</th><th>RNDS</th><th>K</th><th>D</th><th>SCORE</th><th>💰</th>';
    return;
  }
  ...
  // Reset header
  document.querySelector('#sb').closest('table').querySelector('thead tr').innerHTML=
    '<th>#</th><th>PLAYER</th><th>TEAM</th><th>K</th><th>D</th><th>K/D</th><th>SCORE</th><th>💰</th>';
  sorted.forEach((e,i)=>{
    ...
    const isBot=!e.isP;
    tr.innerHTML=`<td>${i===0?'👑':'#'+(i+1)}</td><td>${e.isP?'<b>▶ '+e.name+'</b>':e.name+(isBot?' 🤖':'')}</td><td>${tm}</td><td class="tk">${e.kills}</td><td class="tkd">${e.deaths}</td><td>${kd}</td><td class="ts">${e.score.toLocaleString()}</td><td class="co">${e.coins}</td>`;
    tb.appendChild(tr);
  });
}
...
function endDuelRound(winner){
  ...
  const scoreStr='YOU '+duelRounds.player+' — '+duelRounds.bot+' BOT';
  bannerTxt.innerHTML=(winner==='PLAYER'?'✓ YOU WON ROUND '+duelRound:'✗ BOT WON ROUND '+duelRound)+'<br><span style="font-size:.7em;color:#aaa">'+scoreStr+' | FIRST TO 10</span><br><span id="duel-cd" style="font-size:1.4em">3</span>';
...
function startDuelRound(){
  ...
  if(player){player.dead=false;player.hp=playerMaxHp;player.x=px;player.y=py;player.setGun(GUN_MAP[equippedGunId]||GUN_MAP['pistol']);player.coins=savedCoins;/* keep ownedGuns — purchased guns persist */upHp();upAmmo();hideRespawn();}
  if(duelBot){const bc=duelBot.coins;duelBot.dead=false;duelBot.hp=MAX_HP;duelBot.x=isHost||!isMultiplayer?AW-260:260;duelBot.y=AH/2;duelBot.setGun(randomBotGun(equippedGunId));duelBot.coins=bc;}
...
function upDuelHUD(){
  ...
  if(el)el.textContent='YOU '+duelRounds.player+' — '+duelRounds.bot+' BOT  |  ROUND '+(duelRound+1)+'  |  FIRST TO 10';
}
function upScore(){
...
function upHp(){
  if(!player)return;
  // Show/hide extra hearts based on playerMaxHp
  for(let i=4;i<=5;i++){
    const el=document.getElementById('h'+i);
    if(el)el.style.display=i<=playerMaxHp?'':'none';
  }
  // Toggle empty class for all active hearts
  for(let i=1;i<=playerMaxHp;i++){
    const el=document.getElementById('h'+i);
    if(el)el.classList.toggle('empty',i>player.hp);
  }
  document.getElementById('shld').textContent=player.shield>0?'🛡️'.repeat(player.shield):'';
}
function upAmmo(){
  if(!player)return;
  const g=player.gun;
...
function upPow(){
  ...
  if(player.powSpd)el.insertAdjacentHTML('beforeend','<span title="Speed">⚡</span>');
  if(player.powDmg)el.insertAdjacentHTML('beforeend','<span title="Damage">🔥</span>');
}
let feedList2=[];
function addFeed(k,v,team){
  feedList2.unshift({k,v,team,life:4.5});
  if(feedList2.length>5)feedList2.pop();
  document.getElementById('feed').innerHTML=feedList2.map(f=>`<div class="fi ${f.team==='red'?'r':f.team==='blue'?'b':'f'}"><span style="color:#f0f0ff">${f.k.slice(0,9)}</span><span style="color:var(--y)"> ⚔ </span><span style="color:#5a5a90">${f.v.slice(0,9)}</span></div>`).join('');
}
...
function buildGunGrid(){
  ...
  grid.innerHTML='';
  const barGuns=GUNS.filter(g=>g.price>0&&(g.id!=='godgun'||ownedGuns.has('godgun')));
  barGuns.forEach(g=>{
    ...
    const bars=n=>'█'.repeat(Math.min(5,Math.round(n)))+'░'.repeat(Math.max(0,5-Math.min(5,Math.round(n))));
    card.innerHTML=`
      <span class="shop-gun-name" style="color:${g.col}">${g.name}</span>
      <span class="shop-price">${g.price===0?'✓ FREE':'💰 '+g.price}</span>
...
function buildItemGrid(){
  ...
  POWS.forEach(pow=>{
    ...
    card.innerHTML=`
      ...
      <span class="shop-item-price">💰 ${pow.price}</span>
...
function _itemDesc(id){
  return {
    ...
    damage:'Deal 2× damage for 5s',
    shield:'Absorb 2 hits',
    ammo:'Instantly reload your gun',
  }[id]||'';
}
function buildShop(){
  document.getElementById('shop-coins-disp').textContent='💰 '+(player?player.coins:0)+' COINS';
...
function buyItem(id){
  ...
  if(player.coins<pow.price){
    showShopMsg('✗ NEED '+pow.price+' COINS','#FF3131');return;
  }
  player.coins-=pow.price;
  applyPow(player,pow);
  upCoins();buildShop();
  showShopMsg('✓ USED '+pow.lbl+'!','#00E5A0');
...
function buyGun(gid){
  ...
  if(owned||gid==='pistol'){
    ...
    equippedGunId=gid;
    upAmmo();buildGunGrid();
    showShopMsg('✓ EQUIPPED '+g.name+'!','#00E5A0');
    _savePersistent();
    if(isMultiplayer)peerSend({type:'gun',gun:gid});
    return;
  }
  if(player.coins<g.price){
    showShopMsg('✗ NEED '+g.price+' COINS (have '+player.coins+')','#FF3131');
    return;
  }
  ...
  document.getElementById('shop-coins-disp').textContent='💰 '+player.coins+' COINS';
  showShopMsg('✓ BOUGHT '+g.name+'!','#00E5A0');
  _savePersistent(); // persist coins + new gun ownership
  _guestSave();       // also save for guests
  if(isMultiplayer)peerSend({type:'gun',gun:gid});
  buildMenuShop();    // refresh menu shop so owned status updates
}
...
function render(){
  ...
  if(mode.id==='ctf'){
    for(const[team,f]of Object.entries(flags)){
      ...
      ctx.fillText(team==='red'?'🚩':'🏳',0,0);ctx.restore();
...
// ═══════════════════════════════════
//  WEBSOCKET ROOM SYSTEM (connects to rivals_server.py)
// ═══════════════════════════════════
let ws=null, myPidServer=null, isHost=false, roomCode='';
...
function _wsConnect(onOpen){
  ...
  ws.onopen=()=>{_wsConnecting=false;onOpen&&onOpen();};
  ws.onmessage=e=>{try{handlePeer(JSON.parse(e.data));}catch(_){}};
  ws.onclose=()=>{ws=null;_wsConnecting=false;roomStatus('Disconnected — reload to reconnect','#FF3131');};
  ws.onerror=()=>{roomStatus('✗ Cannot connect to server','#FF3131');};
}
 
// peerSend defined below in P2P section
 
function roomStatus(msg,col){
  const el=document.getElementById('room-status');
  if(el){el.textContent=msg;el.style.color=col||'#FFE600';}
}
 
// HOST: creates a room on the server
// NOTE: makeRoom() and joinRoom() are defined below using PeerJS (P2P) —
// this file previously had an old WebSocket-based duplicate here that was
// dead code (silently overridden by the real versions). Removed to avoid confusion.
 
function hostStartGame(){
  lobbyHide();
  document.getElementById('lobby-start-btn').style.display='none';
  if(Object.keys(_peerConns).length>0){
    isMultiplayer=true;
    _pBroadcast({type:'gameStarted'});
  }
  startGame();
}
 
// ═══════════════════════════════════════════════════════
//  P2P ROOM SYSTEM  (PeerJS — no server needed)
// ═══════════════════════════════════════════════════════
// Replace the entire WS block with PeerJS P2P.
// Host peer id = 'rivals-XXXX', guest id = 'rivals-XXXX-g-<ts>'
// myPidServer = local string pid used as player key in remotePlayers
 
let _peerObj = null;          // PeerJS Peer instance
let _peerConns = {};          // pid -> DataConnection
let _peerLibLoaded = false;
 
function _loadPeerLib(cb){
  if(window.Peer){cb();return;}
  if(_peerLibLoaded){// already loading
    const wait=setInterval(()=>{if(window.Peer){clearInterval(wait);cb();}},100);return;}
  _peerLibLoaded=true;
  const s=document.createElement('script');
  s.src='https://unpkg.com/peerjs@1.5.2/dist/peerjs.min.js';
  s.onload=cb;
  s.onerror=()=>roomStatus('✗ Could not load PeerJS (check internet)','#FF3131');
  document.head.appendChild(s);
}
 
function _pBroadcast(obj){
  const s=JSON.stringify(obj);
  Object.values(_peerConns).forEach(c=>{try{if(c&&c.open)c.send(s);}catch(e){}});
}
function _pSendTo(conn,obj){try{if(conn&&conn.open)conn.send(JSON.stringify(obj));}catch(e){}}
 
function peerSend(obj){
  if(!obj.pid)obj.pid=myPidServer;
  if(obj.type==='kill'){
    obj.killer_pid=myPidServer;
    obj.killer_name=obj.name||playerName;
    obj.victim_name=obj.victim||'?';
    obj.killer_team=obj.team||myTeam;
  }
  _pBroadcast(obj);
}
...
function makeRoom(){
  ...
  const code=raw&&/^\d{4}$/.test(raw)?raw:String(Math.floor(1000+Math.random()*9000));
  roomCode=code; isHost=true;
  document.getElementById('code-in').value=code;
  roomStatus('⏳ Opening room…');
  _loadPeerLib(()=>{
    if(_peerObj)try{_peerObj.destroy();}catch(e){}
    _peerConns={};
    myPidServer='rivals-'+code;
    _peerObj=new Peer(myPidServer,{debug:0});
    _peerObj.on('open',()=>{
      const el=document.getElementById('room-code-big');
      if(el){el.style.display='block';el.textContent=code;}
      roomStatus('✓ ROOM '+code+' OPEN — share this code!','#00E5A0');
      lobbyHide();
      lobbyAddPlayer('me',playerName,myTeam,mySkin.id,true);
      lobbyChatAdd('SYSTEM','Room '+code+' created — share this code!',false);
      lobbyShow();
      switchTab('play');
      const sb=document.getElementById('lobby-start-btn');if(sb)sb.style.display='block';
    });
    _peerObj.on('connection',conn=>{
      const gid=conn.peer;
      _peerConns[gid]=conn;
      conn.on('open',()=>{
        // send welcome with current lobby state
        _pSendTo(conn,{type:'roomJoined',code:roomCode,pid:gid,
          hostName:playerName,hostTeam:myTeam,hostSkin:mySkin.id,mode:mode.id});
      });
      conn.on('data',d=>{try{_handleP2P(JSON.parse(d),gid);}catch(e){}});
      conn.on('close',()=>{
        const k=gid;
        delete _peerConns[k];
        lobbyRemovePlayer(k);
        delete remotePlayers[k];
        lobbyChatAdd('SYSTEM','A player left.',false);
        addFeed('PLAYER','LEFT','ffa');
      });
    });
    _peerObj.on('error',e=>{
      if(e.type==='unavailable-id'){
        ...
        roomStatus('Code taken — use: '+nc,'#FF3131');
      } else roomStatus('✗ '+e.message,'#FF3131');
    });
  });
}
...
function joinRoom(){
  ...
  if(!/^\d{4}$/.test(code)){roomStatus('✗ Enter 4-digit code','#FF3131');return;}
  roomCode=code; isHost=false;
  roomStatus('⏳ Connecting to room '+code+'…');
  _loadPeerLib(()=>{
    if(_peerObj)try{_peerObj.destroy();}catch(e){}
    _peerConns={};
    myPidServer='rivals-'+code+'-g-'+Date.now();
    _peerObj=new Peer(myPidServer,{debug:0});
    _peerObj.on('open',()=>{
      const conn=_peerObj.connect('rivals-'+code,{reliable:true});
      _peerConns['host']=conn;
      conn.on('open',()=>{
        conn.send(JSON.stringify({type:'guestHello',pid:myPidServer,
          name:playerName,team:myTeam,skin:mySkin.id}));
      });
      conn.on('data',d=>{try{_handleP2P(JSON.parse(d),'host');}catch(e){}});
      conn.on('close',()=>{
        delete _peerConns['host'];
        roomStatus('Host disconnected','#FF3131');
        if(running)addFeed('HOST','LEFT','ffa');
      });
    });
    _peerObj.on('error',e=>{
      if(e.type==='peer-unavailable')roomStatus('✗ Room '+code+' not found','#FF3131');
      else roomStatus('✗ '+e.message,'#FF3131');
    });
  });
}
...
function _handleP2P(msg, fromId){
  switch(msg.type){
 
    // ── LOBBY ──────────────────────────────────────────
 
    case'guestHello':{
      // Host receives new guest
      const k=msg.pid;
      const guestTeam=myTeam==='red'?'blue':'red';
      lobbyAddPlayer(k,msg.name,guestTeam,msg.skin||'phantom',false);
      lobbyChatAdd('SYSTEM',msg.name+' joined the room!',false);
      remotePlayers[k]={x:guestTeam==='red'?100:AW-100,y:AH/2,angle:0,
        name:msg.name,team:guestTeam,skin:msg.skin||'phantom',
        hp:3,dead:false,gun:'pistol',kills:0,deaths:0,score:0,coins:0,vx:0,vy:0};
      addFeed(msg.name,'JOINED',guestTeam);
      // Tell everyone about the new player
      _pBroadcast({type:'playerJoined',pid:k,name:msg.name,team:guestTeam,skin:msg.skin||'phantom'});
      break;
    }
 
    case'roomJoined':{
      // Guest receives confirmation from host
      isMultiplayer=true;
      roomStatus('✓ Joined room '+msg.code+' — waiting for host…','#00E5A0');
      const hostTeam=msg.hostTeam||'red';
      const myGuestTeam=hostTeam==='red'?'blue':'red';
      myTeam=myGuestTeam;
      document.getElementById('tbr').classList.toggle('on',myTeam==='red');
      document.getElementById('tbb').classList.toggle('on',myTeam==='blue');
      lobbyHide();
      lobbyAddPlayer('host',msg.hostName||'HOST',hostTeam,msg.hostSkin||'phantom',false);
      lobbyAddPlayer('me',playerName,myGuestTeam,mySkin.id,false);
      lobbyChatAdd('SYSTEM','Joined room '+msg.code+'!',false);
      lobbyShow();
      showScreen('menu');switchTab('play');
      document.querySelector('.wrap').classList.add('guest-locked');
      // Set up host as remote player
      remotePlayers['host']={x:hostTeam==='red'?100:AW-100,y:AH/2,angle:0,
        name:msg.hostName||'HOST',team:hostTeam,skin:msg.hostSkin||'phantom',
        hp:3,dead:false,gun:'pistol',kills:0,deaths:0,score:0,coins:0,vx:0,vy:0};
      break;
    }
 
    case'playerJoined':{
      const k=msg.pid; if(k===myPidServer)break;
      if(!remotePlayers[k])remotePlayers[k]={x:msg.team==='red'?100:AW-100,y:AH/2,angle:0,
        name:msg.name,team:msg.team,skin:msg.skin||'phantom',
        hp:3,dead:false,gun:'pistol',kills:0,deaths:0,score:0,coins:0,vx:0,vy:0};
      lobbyAddPlayer(k,msg.name,msg.team,msg.skin||'phantom',false);
      lobbyChatAdd('SYSTEM',msg.name+' joined!',false);
      break;
    }
 
    case'gameStarted':
      lobbyHide(); isMultiplayer=true;
      roomStatus('✓ Game started!','#00E5A0');
      if(!running)startGame();
      break;
 
    case'chat':
      if(msg.pid!==myPidServer)lobbyChatAdd(msg.name||'?',msg.text||'',false);
      // Host relays to all
      if(isHost)_pBroadcast({type:'chat',pid:msg.pid,name:msg.name,text:msg.text});
      break;
    ...
    case'state':{
      const k=msg.pid; if(k===myPidServer)break;
      if(!remotePlayers[k])break;
      remotePlayers[k].x=msg.x; remotePlayers[k].y=msg.y;
      remotePlayers[k].angle=msg.angle;
      remotePlayers[k].vx=msg.vx||0; remotePlayers[k].vy=msg.vy||0;
      break;
    }
    ...
    case'shoot':{
      ...
      if(rp&&!rp.dead){
        for(let i=0;i<g.pel;i++){
          const sp=(Math.random()-.5)*g.spread*2+(g.pel>1?(i-2)*.06:0);
          const fo={x:msg.x,y:msg.y,team:rp.team,powDmg:false,vx:0,vy:0};
          bullets.push(new Bullet(msg.x,msg.y,msg.angle+sp,fo,g));
        }
      }
      break;
    }
    ...
    case'kill':{
      const k=msg.killer_pid||msg.pid;
      if(remotePlayers[k]){
        remotePlayers[k].kills=(remotePlayers[k].kills||0)+1;
        remotePlayers[k].score=(remotePlayers[k].score||0)+100;
        remotePlayers[k].coins=(remotePlayers[k].coins||0)+5;
      }
      addFeed(msg.killer_name||msg.name||'?',msg.victim_name||msg.victim||'?',msg.killer_team||msg.team||'ffa');
      const kt=msg.killer_team||msg.team;
      if(kt==='red'||kt==='blue'){kills[kt]=(kills[kt]||0)+1;}
      upScore();
      break;
    }
    ...
    case'hp':{
      const k=msg.pid; if(k===myPidServer)break;
      if(remotePlayers[k]){remotePlayers[k].hp=msg.hp;if(msg.hp<=0)remotePlayers[k].dead=true;}
      break;
    }
 
    case'dead':{
      const k=msg.pid; if(k===myPidServer)break;
      if(remotePlayers[k]){remotePlayers[k].dead=true;remotePlayers[k].hp=0;}
      if(mode.id==='1v1'&&!gameOver&&!duelBetween&&performance.now()-gameStartT>2000){
        duelRounds.player++;endDuelRound('PLAYER');
      }
      break;
    ...
    case'respawn':{
      const k=msg.pid; if(k===myPidServer)break;
      if(remotePlayers[k]){
        remotePlayers[k].dead=false;remotePlayers[k].hp=3;
        remotePlayers[k].x=remotePlayers[k].team==='red'?100:AW-100;
        remotePlayers[k].y=AH/2;
      }
      break;
    }
    ...
    case'gun':{
      const k=msg.pid; if(k===myPidServer)break;
      if(remotePlayers[k])remotePlayers[k].gun=msg.gun;
      break;
    }
    ...
    case'score':{
      const k=msg.pid; if(k===myPidServer)break;
      if(remotePlayers[k]){
        if(msg.kills!==undefined)remotePlayers[k].kills=msg.kills;
        if(msg.score!==undefined)remotePlayers[k].score=msg.score;
        if(msg.coins!==undefined)remotePlayers[k].coins=msg.coins;
      }
      upScore();
      break;
    }
    ...
    case'duelNewRound':{
      const k=Object.keys(remotePlayers)[0];
      if(k&&remotePlayers[k]){
        remotePlayers[k].dead=false;remotePlayers[k].hp=3;
        remotePlayers[k].gun='pistol';
        remotePlayers[k].x=remotePlayers[k].team==='red'?260:AW-260;
        remotePlayers[k].y=AH/2;
      }
      break;
    }
...
async function startGame(){
  ...
  document.getElementById('mode-tag').textContent={tdm:'⚔️ TDM',ffa:'💀 FFA',koth:'👑 KOTH',ctf:'🚩 CTF',gun:'🔫 GUN GAME','1v1':'🥊 1V1',ccs:'🏛️ CONTROL'}[mode.id];
  ...
  gameOver=false;shakeMag=0;pup_t=0;paused=false;shopOpen=false;hideWinScreen();
  ...
  if(_authUser&&_authUser.stats){
    ...
    if(_authUser.stats.equippedGunId&&GUN_MAP[_authUser.stats.equippedGunId]){
      equippedGunId=_authUser.stats.equippedGunId;
    }
  } else {
    ownedGuns=_guestOwnedGuns(); // load guest-saved guns
    const gd=_guestLoad();
    if(gd&&gd.equippedGunId&&GUN_MAP[gd.equippedGunId]&&(ownedGuns.has(gd.equippedGunId)||gd.equippedGunId==='pistol')){
      equippedGunId=gd.equippedGunId;
    }
    if(gd&&gd.playerMaxHp&&gd.playerMaxHp>3){playerMaxHp=Math.min(5,gd.playerMaxHp);}
  }
  ...
  if(mode.id==='1v1'){
    ...
    player=new Entity(px,py,myTeam,playerName,true,mySkin,GUN_MAP[equippedGunId]||GUN_MAP['pistol']);
    ffaK[playerName]=0;
    ents.push(player);
    // Load saved coin bank (account or guest)
    if(_authUser&&_authUser.stats&&_authUser.stats.coins)player.coins=_authUser.stats.coins;
    else{const gc=_guestLoad();if(gc){player.coins=gc.coins||0;}}
    // Always spawn a bot placeholder — it gets removed when a human joins
    ...
    const bot=new Entity(botSide,py,botTeam,'RIVAL',false,botSkin,randomBotGun(equippedGunId));
    ...
    // Only fill the match with AI bots in local/solo play — in a real
    // multiplayer room, other humans fill those slots instead.
    if(!isMultiplayer){
      for(let i=0;i<5;i++){
        ...
        const bot=new Entity(bx,by,t,nm,false,sk,randomBotGun(equippedGunId));
...
// ═══════════════════════════════════
function pickTeam(t){
  if(isMultiplayer && !isHost) return; // guest can't pick team
  myTeam=t;
  document.getElementById('tbr').classList.toggle('on',t==='red');
  document.getElementById('tbb').classList.toggle('on',t==='blue');
  // Tell guest their forced team
  if(isMultiplayer && isHost){
    const guestTeam = t==='red'?'blue':'red';
    peerSend({type:'hostSetTeam', hostTeam:t, guestTeam:guestTeam});
    lobbyChatAdd('SYSTEM','Host picked '+t.toUpperCase()+' — you are '+guestTeam.toUpperCase(), false);
  }
}
function switchTab(id){
  ...
  if(id==='shop')buildMenuShop();
  if(id==='inventory')buildInvWeapons();
}
 
function buildInvWeapons(){
  const grid=document.getElementById('inv-gun-cards');
  if(!grid)return;
  grid.innerHTML='';
  // Get the owned set (works outside game too)
  let owned;
  if(player&&!gameOver) owned=ownedGuns;
  else if(_authUser&&_authUser.stats) owned=new Set(_authUser.stats.ownedGuns||['pistol']);
  else owned=_guestOwnedGuns()||new Set(['pistol']);
 
  const myGuns=GUNS.filter(g=>owned.has(g.id)||g.price===0);
  // Always include pistol
  if(!myGuns.find(g=>g.id==='pistol')) myGuns.unshift(GUN_MAP['pistol']);
 
  myGuns.forEach(g=>{
    const isEquipped=equippedGunId===g.id;
    const d=document.createElement('div');
    d.className='gun-card';
    d.style.border='2px solid '+(isEquipped?'#FFE600':'var(--bdr)');
    d.style.background=isEquipped?'rgba(255,230,0,.07)':'var(--bg)';
    const stars=n=>'█'.repeat(Math.min(5,Math.round(n)))+'░'.repeat(Math.max(0,5-Math.min(5,Math.round(n))));
    const dmgN=g.dmg*(g.pel||1)/(g.rate||1)*.3;
    const spdN=(g.spd||500)/300;
    const ammoN=(g.ammo||1)/20;
    d.innerHTML=`
      <span class="gn" style="color:${g.col}">${g.id==='godgun'?'⚡ ':''} ${g.name}</span>
      <span class="gs">${g.desc||''}</span>
      <span class="gs" style="color:#3a3a60">DMG ${stars(g.dmg*g.pel*1.2)} SPD ${stars(spdN)} AMMO ${stars(ammoN)}</span>
      ${g.id==='godgun'?'<span class="gp" style="color:#FFE600;font-size:.22rem">⚡ CHEAT CODE GUN</span>':''}
      ${isEquipped
        ? '<div style="font-family:\'Press Start 2P\',monospace;font-size:.26rem;color:#FFE600;margin-top:.5rem;padding:.3rem;background:rgba(255,230,0,.1);text-align:center">✓ EQUIPPED</div>'
        : `<button onclick="invEquipGun('${g.id}')" style="width:100%;margin-top:.5rem;font-family:'Press Start 2P',monospace;font-size:.26rem;padding:.32rem;background:transparent;border:1px solid #00E5A0;color:#00E5A0;cursor:pointer" onmouseover="this.style.background='rgba(0,229,160,.15)'" onmouseout="this.style.background='transparent'">EQUIP</button>`
      }
    `;
    grid.appendChild(d);
  });
  ...
  if(myGuns.length===0){
    grid.innerHTML='<div style="font-family:\'Press Start 2P\',monospace;font-size:.3rem;color:#5a5a90;padding:1rem">No weapons owned yet.<br><br>Earn coins in matches<br>and visit the SHOP!</div>';
  }
}
 
function invEquipGun(gid){
  const g=GUN_MAP[gid];
  if(!g)return;
  equippedGunId=gid;
  // If in game, switch live too
  if(player&&!player.dead&&running){
    player.setGun(g);
    upAmmo();
    buildGunGrid();
  }
  buildInvWeapons(); // refresh to show new equipped state
  _guestSave();
  _savePersistent();
}
function buildMenu(){
  ...
  MODES.forEach((m,i)=>{
    ...
    d.onclick=()=>{
      if(isMultiplayer && !isHost) return; // guest can't pick mode
      mode=m;
      document.querySelectorAll('.mode-btn').forEach(b=>b.classList.remove('on'));
      d.classList.add('on');
      // Tell guest which mode host picked
      if(isMultiplayer && isHost){
        peerSend({type:'hostSetMode', modeId:m.id, modeName:m.name});
        lobbyChatAdd('SYSTEM','Host set mode to '+m.name, false);
      }
    };
    ml.appendChild(d);
  });
  ...
  if(sg){
    SKINS.forEach((s,i)=>{
      ...
      d.innerHTML='<span class="se">'+s.emoji+'</span><span class="sn">'+s.name+'</span><span class="ck">✓</span>';
      d.onclick=()=>{
        const prev=mySkin;
        mySkin=s;
        document.querySelectorAll('.skin-card').forEach(b=>b.classList.remove('on'));
        d.classList.add('on');
        // Update lobby player card if in a room
        if(Object.keys(lobbyPlayers).length>0){
          lobbyAddPlayer('me', playerName||'YOU', myTeam, s.id, isHost);
          // Announce skin change in lobby chat
          const msg=prev.emoji+' → '+s.emoji+' '+(playerName||'YOU')+' switched to '+s.name;
          lobbyChatAdd('SKIN', msg, true);
          peerSend({type:'skinChange', name:playerName||'YOU', skin:s.id, emoji:s.emoji, skinName:s.name, prevEmoji:prev.emoji});
        }
      };
      sg.appendChild(d);
    });
...
function _menuCoins(){
  // Get coins from player (in-game), auth account, or guest save
  if(player&&!gameOver)return player.coins;
  if(_authUser&&_authUser.stats)return _authUser.stats.coins||0;
  return _guestCoins()||0;
}
 
function buildMenuShop(){
  const gc=document.getElementById('gun-cards-menu');
  if(!gc)return;
  gc.innerHTML='';
  const coins=_menuCoins();
  const cd=document.getElementById('menu-coins-disp');
  if(cd)cd.textContent='💰 '+coins+' COINS';
  let owned;
  if(player&&!gameOver)owned=ownedGuns;
  else if(_authUser&&_authUser.stats)owned=new Set(_authUser.stats.ownedGuns||['pistol']);
  else owned=_guestOwnedGuns();
  const stars=n=>'█'.repeat(Math.min(5,Math.round(n)))+'░'.repeat(Math.max(0,5-Math.min(5,Math.round(n))));
  GUNS.filter(g=>g.id!=='godgun'||ownedGuns.has('godgun')).forEach(g=>{
    const isOwned=owned.has(g.id)||g.price===0||g.price===-1;
    const canAfford=coins>=g.price;
    const d=document.createElement('div');
    d.className='gun-card';
    d.style.border='2px solid '+(isOwned?'#00E5A0':'var(--bdr)');
    d.style.background=isOwned?'rgba(0,229,160,.05)':'var(--bg)';
    let btnHtml='';
    if(g.price===0){
      btnHtml='<div style="font-family:monospace;font-size:.7rem;color:#00E5A0;margin-top:.5rem;padding:.35rem;background:rgba(0,229,160,.1)">✓ FREE</div>';
    } else if(isOwned){
      btnHtml='<div style="font-family:monospace;font-size:.7rem;color:#00E5A0;margin-top:.5rem;padding:.35rem;background:rgba(0,229,160,.1)">✓ OWNED</div>';
    } else {
      const btn=document.createElement('button');
      btn.disabled=!canAfford;
      btn.textContent='BUY '+g.price+' 💰';
      btn.style.cssText='font-family:monospace;font-size:.7rem;background:'+(canAfford?'var(--y)':'var(--bdr)')+';color:'+(canAfford?'#000':'#555')+';border:none;cursor:'+(canAfford?'pointer':'not-allowed')+';padding:.35rem;margin-top:.5rem;width:100%;transition:filter .15s';
      if(canAfford){btn.onmouseover=function(){this.style.filter='brightness(1.1)'};btn.onmouseout=function(){this.style.filter=''};}
      btn.onclick=function(){menuBuyGun(g.id);};
      d.innerHTML='<span class="gn" style="color:'+g.col+'">'+g.name+'</span>'
        ...
        +'<span class="gp" style="color:#FFE600">💰 '+g.price+' COINS</span>';
      d.appendChild(btn);
      gc.appendChild(d);
      return;
    }
    d.innerHTML='<span class="gn" style="color:'+g.col+'">'+g.name+'</span>'
      ...
      +'<div class="gs">AMMO '+g.ammo+' | RELOAD '+g.reload+'s</div>'
      +'<div class="gs" style="font-size:.75rem;color:#3a3a60;margin-top:.2rem">'+g.desc+'</div>'
      +'<span class="gp" style="color:'+(isOwned?'#00E5A0':'#FFE600')+'">'+(g.price===0?'✓ FREE':'💰 '+g.price)+' COINS</span>'
      +btnHtml;
    gc.appendChild(d);
  });
}
 
function menuBuyGun(gid){
  const g=GUN_MAP[gid];if(!g||g.price===0)return;
  // Determine which store to deduct from
  let coins=_menuCoins();
  if(coins<g.price){
    alert('✗ Need '+g.price+' coins — you have '+coins);return;
  }
  // Deduct and save
  if(_authUser&&_authUser.stats){
    _authUser.stats.coins-=g.price;
    _authUser.stats.ownedGuns=_authUser.stats.ownedGuns||['pistol'];
    if(!_authUser.stats.ownedGuns.includes(gid))_authUser.stats.ownedGuns.push(gid);
    const accts=_accts();
    if(accts[_authUser.username]){
      accts[_authUser.username].stats.coins=_authUser.stats.coins;
      accts[_authUser.username].stats.ownedGuns=_authUser.stats.ownedGuns;
      _saveAccts(accts);
    }
    ownedGuns.add(gid);
    _applyLoginToMenu();
    ...
    // Guest
    const gd=_guestLoad()||{coins:0,ownedGuns:['pistol']};
    gd.coins=Math.max(0,gd.coins-g.price);
    if(!gd.ownedGuns.includes(gid))gd.ownedGuns.push(gid);
    try{localStorage.setItem('rivals_guest',JSON.stringify(gd));}catch(e){}
    ownedGuns.add(gid);
  }
  // Also apply to live player if in-game
  if(player&&!gameOver){
    player.coins-=g.price;
    player.setGun(g);
    ownedGuns.add(gid);
...
// ── LOBBY PANEL ─────────────────────────────────────────────────────────────
const lobbyPlayers = {}; // pid/key -> {name, team, skin, isMe}
...
function lobbyHide(){
  ...
  Object.keys(lobbyPlayers).forEach(k=>delete lobbyPlayers[k]);
  document.getElementById('lobby-player-list').innerHTML='';
  document.getElementById('lobby-chat-msgs').innerHTML='';
}
function lobbyAddPlayer(key, name, team, skinId, isMe){
  lobbyPlayers[key]={name,team,skinId,isMe};
  lobbyRender();
}
function lobbyRemovePlayer(key){
  delete lobbyPlayers[key];
  lobbyRender();
}
function lobbyRender(){
  const el=document.getElementById('lobby-player-list');
  el.innerHTML='';
  Object.entries(lobbyPlayers).forEach(([key,p])=>{
    const skin=SKINS.find(s=>s.id===p.skinId)||SKINS[0];
    const teamCol=p.team==='red'?'var(--r)':'var(--b)';
    const teamLbl=p.team==='red'?'🔴 RED':'🔵 BLUE';
    const d=document.createElement('div');
    d.className='lp-card';
    d.innerHTML=`<span class="lp-emoji">${skin.emoji}</span>
      <div class="lp-info">
        <div class="lp-name" style="color:${skin.col}">${p.name}${p.isMe?' (YOU)':''}</div>
        <div class="lp-sub">${teamLbl}</div>
      </div>
      ${p.isMe?'<span class="lp-badge" style="border-color:var(--y);color:var(--y)">HOST</span>':'<span class="lp-badge" style="border-color:var(--g);color:var(--g)">JOINED</span>'}`;
    el.appendChild(d);
  });
}
function lobbyChatAdd(who, text, isMe){
  const el=document.getElementById('lobby-chat-msgs');
  const d=document.createElement('div');
  d.className='lc-msg';
  const whoClass=who==='SKIN'?'skin':who==='SYSTEM'?'them':isMe?'me':'them';
  d.innerHTML=`<span class="lc-who ${whoClass}">${who==='SKIN'?'👗':who==='SYSTEM'?'🔔':who}</span><span style="color:#c0c0e0">${text}</span>`;
  el.appendChild(d);
...
function lobbyChatSend(){
  ...
  if(!txt)return;
  inp.value='';
  lobbyChatAdd(playerName||'YOU', txt, true);
  peerSend({type:'lobbyChat', name:playerName||'YOU', text:txt});
}
// Enter key to send
document.addEventListener('DOMContentLoaded',()=>{
  const inp=document.getElementById('lobby-chat-input');
  if(inp)inp.addEventListener('keydown',e=>{if(e.key==='Enter')lobbyChatSend();});
});
 
 
// ── WIN SCREEN ────────────────────────────────────────────────────────────────
function showWinScreen(won, kills, deaths){
  const ws = document.getElementById('win-screen');
  const lbl = document.getElementById('win-label');
  const killEl = document.getElementById('win-kills');
  const crown = document.getElementById('win-crown');
 
  if(won){
    lbl.textContent = 'YOU WIN!';
    lbl.style.color = 'var(--y)';
    lbl.style.textShadow = '0 0 40px rgba(255,230,0,.9)';
    crown.textContent = '🏆';
  } else {
    lbl.textContent = 'YOU LOSE';
    lbl.style.color = 'var(--r)';
    lbl.style.textShadow = '0 0 40px rgba(255,49,49,.9)';
    crown.textContent = '💀';
  }
  killEl.innerHTML = '<span style="color:var(--y)">K: '+kills+'</span>  <span style="color:#5a5a90">D: '+deaths+'</span>';
  killEl.style.color = '#aaa';
 
  // Confetti burst for winner
  if(won){
    const pc = document.getElementById('win-particles');
    pc.innerHTML = '';
    const colors = ['#FFE600','#FF3131','#1E90FF','#00E5A0','#FF0080'];
    for(let i=0;i<60;i++){
      const p=document.createElement('div');
      const col=colors[Math.floor(Math.random()*colors.length)];
      const x=Math.random()*100, delay=Math.random()*1.5, dur=1.5+Math.random()*2;
      p.style.cssText='position:absolute;width:8px;height:8px;background:'+col+';left:'+x+'vw;top:-10px;border-radius:'+Math.random()*4+'px;animation:conffall '+dur+'s '+delay+'s linear forwards';
      pc.appendChild(p);
    }
    // Add confetti animation if not already there
    if(!document.getElementById('conffall-style')){
...
const JOY_MAX = 45; // max radius from center
 
function joyShow(side, x, y){
  const base  = document.getElementById('joy-'+side+'-base');
  const stick = document.getElementById('joy-'+side+'-stick');
...
function joyMove(side, x, y, bx, by){
  const stick = document.getElementById('joy-'+side+'-stick');
  ...
  stick.style.left = nx+'px';
  stick.style.top  = ny+'px';
}
function joyHide(side){
  document.getElementById('joy-'+side+'-base').style.display='none';
  document.getElementById('joy-'+side+'-stick').style.display='none';
}
 
// Prevent default touch to stop scroll/zoom during game
document.addEventListener('touchmove', e=>{
  if(document.getElementById('game').classList.contains('active')) e.preventDefault();
},{passive:false});
 
document.addEventListener('touchstart', e=>{
  if(!document.getElementById('game').classList.contains('active')) return;
  for(const t of e.changedTouches){
    const lz = document.getElementById('joy-left-zone').getBoundingClientRect();
    const rz = document.getElementById('joy-right-zone').getBoundingClientRect();
    const inLeft  = t.clientX >= lz.left && t.clientX <= lz.right  && t.clientY >= lz.top && t.clientY <= lz.bottom;
    const inRight = t.clientX >= rz.left && t.clientX <= rz.right  && t.clientY >= rz.top && t.clientY <= rz.bottom;
    if(inLeft && !joyL.active){
      joyL.active=true; joyL.id=t.identifier;
      joyL.bx=t.clientX; joyL.by=t.clientY;
      joyL.dx=0; joyL.dy=0;
      joyShow('left', t.clientX, t.clientY);
    } else if(inRight && !joyR.active){
      joyR.active=true; joyR.id=t.identifier;
      joyR.bx=t.clientX; joyR.by=t.clientY;
      joyR.dx=0; joyR.dy=0;
      joyShow('right', t.clientX, t.clientY);
      mdown=true;
    }
  }
},{passive:false});
...
document.addEventListener('touchmove', e=>{
  ...
  for(const t of e.changedTouches){
    if(joyL.active && t.identifier===joyL.id){
      const dx=t.clientX-joyL.bx, dy=t.clientY-joyL.by;
      const dist=Math.hypot(dx,dy)||1;
      const capped=Math.min(JOY_MAX,dist);
      joyL.dx=(dx/dist)*(capped/JOY_MAX);
      joyL.dy=(dy/dist)*(capped/JOY_MAX);
      joyMove('left', t.clientX, t.clientY, joyL.bx, joyL.by);
    }
    if(joyR.active && t.identifier===joyR.id){
      const dx=t.clientX-joyR.bx, dy=t.clientY-joyR.by;
      const dist=Math.hypot(dx,dy)||1;
      joyR.dx=dx/dist; joyR.dy=dy/dist;
      joyMove('right', t.clientX, t.clientY, joyR.bx, joyR.by);
      // Aim player toward right stick direction
      if(player && !player.dead && dist>8){
        player.angle = Math.atan2(dy, dx);
        // Update mouse position so shooting works
        const cx = (player.x*scl)+ox;
        const cy = (player.y*scl)+oy;
        mouse.x = cx + dx*3;
        mouse.y = cy + dy*3;
      }
    }
...
// Inject joystick movement into player update
// We patch keys via a proxy — read joyL in player update
const _origUpdate = Entity.prototype.update;
Entity.prototype.update = function(dt){
  if(this.isP && isMobile && joyL.active){
    // Override keyboard dx/dy with joystick
    this._mobDx = joyL.dx;
    this._mobDy = joyL.dy;
  } else if(this.isP && isMobile){
    this._mobDx = 0;
    this._mobDy = 0;
  }
...
// Patch the movement section to read _mobDx/_mobDy when on mobile
// We override keys reading for the player only
const _origKeys = {};
function getMobDx(){ return (player && isMobile) ? (player._mobDx||0) : 0; }
function getMobDy(){ return (player && isMobile) ? (player._mobDy||0) : 0; }
 
// Action buttons
document.addEventListener('DOMContentLoaded', ()=>{
  const reloadBtn = document.getElementById('mob-reload');
  const shopBtn   = document.getElementById('mob-shop');
  const swordBtn  = document.getElementById('mob-sword');
 
  if(reloadBtn) reloadBtn.addEventListener('touchstart', e=>{
    e.stopPropagation();
    if(player&&!player.reloading&&player.ammo<player.gun.ammo){
      player.reloading=true; player.reloadT=player.gun.reload;
    }
  },{passive:true});
 
  if(shopBtn) shopBtn.addEventListener('touchstart', e=>{
    e.stopPropagation();
    if(shopOpen) closeShop(); else openShop();
  },{passive:true});
 
  if(swordBtn) swordBtn.addEventListener('touchstart', e=>{
    e.stopPropagation();
    if(player&&!player.dead) player.startAtk();
  },{passive:true});
});
...
function submitCheat(){
  ...
  if(code === 'ven7709'){
    // ── GOD GUN cheat ────────────────────────────────────
    cheatUnlocked = true;
    ownedGuns.add('godgun');
    equippedGunId='godgun'; // auto-equip it!
    if(player&&running&&!player.dead){ player.setGun(GUN_MAP['godgun']); upAmmo(); }
    _guestSave();
    if(_authUser){ // persist for logged-in users too
      _authUser.stats.ownedGuns = [...ownedGuns];
      _savePersistent();
    }
    buildMenuShop();
    buildInvWeapons();
    inp.classList.remove('denied');
    inp.classList.add('activated');
    msg.textContent = '✓ GOD GUN UNLOCKED!';
    msg.style.color = '#00E5A0';
    setTimeout(()=>{inp.classList.remove('activated');msg.textContent='';},2500);
 
  } else if(code === 'testbot'){
    // ── TEST BOT cheat — spawn a bot that appears as a real player ──
    if(!running || gameOver){
      msg.textContent = '✗ START A GAME FIRST';
      msg.style.color = '#FF3131';
      inp.classList.add('denied');
      setTimeout(()=>{inp.classList.remove('denied');msg.textContent='';},1800);
      return;
    }
    // Create TEST BOT — looks like a real player on scoreboard
    const tbSkin = SKINS.find(s=>s.id==='ghost')||SKINS[1];
    const tbTeam = myTeam==='red'?'blue':'red';
    const tbx = tbTeam==='red'?260:AW-260;
    const tby = AH/2;
    const tb = new Entity(tbx, tby, tbTeam, 'TEST BOT', false, tbSkin, randomBotGun(equippedGunId));
    tb.aggro = 0.5;
    tb.isTestBot = true; // flag for special rendering
    ents.push(tb);
    ffaK['TEST BOT'] = 0;
    // Also show as remote player so it appears in multiplayer feed
    if(isMultiplayer){
      remotePlayers['testbot'] = {
        x:tbx, y:tby, angle:0, name:'TEST BOT', team:tbTeam,
        skin:tbSkin.id, hp:3, dead:false, gun:'pistol',
        kills:0, deaths:0, score:0, coins:0, vx:0, vy:0
      };
    }
    inp.classList.remove('denied');
    inp.classList.add('activated');
    msg.textContent = '✓ TEST BOT SPAWNED!';
    msg.style.color = '#00E5A0';
    addFeed('TEST BOT','JOINED',tbTeam);
    setTimeout(()=>{inp.classList.remove('activated');msg.textContent='';},2500);
    ...
    msg.style.color = '#FF3131';
    inp.classList.add('denied');
    setTimeout(()=>{inp.classList.remove('denied');msg.textContent='';},1500);
  }
}
 
// Enter key support
document.addEventListener('DOMContentLoaded',()=>{
  const ci=document.getElementById('cheat-in');
  if(ci)ci.addEventListener('keydown',e=>{if(e.key==='Enter')submitCheat();});
});
...
function goMenu(){
  hideWinScreen();
  lobbyHide();
  const w=document.querySelector('.wrap');if(w)w.classList.remove('guest-locked');
  if(_peerObj)try{_peerObj.destroy();}catch(e){}
  _peerObj=null;_peerConns={};
  remotePlayers={};isMultiplayer=false;roomCode='';myPidServer=null;
  ...
  showScreen('menu');
  buildMenuShop(); // refresh coins after game
}
...
// ═══════════════════════════════════════════════════════
//  AUTH SYSTEM — localStorage, no email, no server
// ═══════════════════════════════════════════════════════
...
function authSubmit(){
  ...
  if(!user){_authMsg('✗ USERNAME REQUIRED');return;}
  if(user.length<2){_authMsg('✗ USERNAME TOO SHORT (min 2)');return;}
  if(pass.length<4){_authMsg('✗ PASSWORD MIN 4 CHARS');return;}
  const accts=_accts();
  if(tab==='signup'){
    const pass2=document.getElementById('auth-pass2').value;
    if(pass!==pass2){_authMsg('✗ PASSWORDS DO NOT MATCH');return;}
    if(accts[user]){_authMsg('✗ USERNAME TAKEN — TRY ANOTHER');return;}
    if(!/^[A-Z0-9_]+$/.test(user)){_authMsg('✗ LETTERS, NUMBERS, _ ONLY');return;}
    accts[user]={hash:_hash(pass+user),stats:{kills:0,deaths:0,wins:0,gamesPlayed:0,totalCoins:0,coins:0,ownedGuns:['pistol']}};
    _saveAccts(accts);
    _authMsg('✓ ACCOUNT CREATED!','#00E5A0');
    setTimeout(()=>_doLogin(user,accts[user]),600);
  } else {
    const acct=accts[user];
    if(!acct){_authMsg('✗ NO ACCOUNT: '+user);return;}
    if(acct.hash!==_hash(pass+user)){_authMsg('✗ WRONG PASSWORD');return;}
    _doLogin(user,acct);
  }
}
...
function authGuest(){
  ...
  equippedGunId=_guestLoad()?.equippedGunId||'pistol';
  _applyLoginToMenu();
  showScreen('menu');
}
...
function _applyLoginToMenu(){
  ...
  if(_authUser){
    ...
    nm.textContent='▶ '+_authUser.username;
    st.textContent='💰'+(s.coins||0)+' · K:'+s.kills+' · D:'+s.deaths+' · WINS:'+s.wins+' · GAMES:'+s.gamesPlayed;
    ...
    st.textContent='STATS NOT SAVED — COINS NOT SAVED';
    document.getElementById('name-in').value='';
    if(logoutBtn)logoutBtn.textContent='SIGN IN / SIGN UP';
  }
}
...
function _savePersistent(){
  ...
  acct.stats.equippedGunId=equippedGunId;
  _authUser.stats.coins=acct.stats.coins;
  _authUser.stats.ownedGuns=acct.stats.ownedGuns;
  _authUser.stats.equippedGunId=equippedGunId;
  _saveAccts(accts);
  _applyLoginToMenu();
}
 
// ── GUEST persistence (no account needed) ──────────────────────────────────
function _guestSave(){
  if(_authUser)return; // logged-in users handled by _savePersistent
  try{
    localStorage.setItem('rivals_guest',JSON.stringify({
      coins: player?player.coins:(_guestCoins()||0),
      ownedGuns:[...ownedGuns],
      equippedGunId: equippedGunId||'pistol',
      playerMaxHp: playerMaxHp
    }));
  }catch(e){}
}
function _guestLoad(){
  try{return JSON.parse(localStorage.getItem('rivals_guest')||'null');}catch(e){return null;}
}
function _guestCoins(){
  const d=_guestLoad();return d?d.coins:0;
}
function _guestOwnedGuns(){
  const d=_guestLoad();return d?new Set(d.ownedGuns||['pistol']):new Set(['pistol']);
}
 
// Hook into endGame to save stats
 
function endScreenReturn(){
  if(isMultiplayer){
    // Go back to lobby — keep peer connection alive
    showScreen('menu');
    switchTab('play');
    lobbyShow();
    // Reset player list to just self, clear chat messages about the game
    Object.keys(lobbyPlayers).forEach(k=>delete lobbyPlayers[k]);
    lobbyAddPlayer('me', playerName, myTeam, mySkin.id, isHost);
    if(Object.keys(_peerConns).length>0){
      // Tell friend we're back in lobby
      peerSend({type:'backToLobby', name:playerName});
      // Re-show start button if host
      if(isHost){
        const btn=document.getElementById('lobby-start-btn');
        if(btn)btn.style.display='block';
      }
    }
    document.getElementById('room-status').textContent=isHost?'✓ Back in lobby — click START GAME when ready':'✓ Back in lobby — waiting for host…';
  } else {
    goMenu();
  }
}
...
function endGame(txt,cls){
  _endGameCore(txt,cls);
  if(gameOver){
    const won=(cls==='y')||(cls==='r'&&myTeam==='red')||(cls==='b'&&myTeam==='blue');
    _saveStats(won);
    _guestSave();
    // Show big win screen for multiplayer games
    if(isMultiplayer){
      const k=player?player.kills:0;
      const d=player?player.deaths:0;
      setTimeout(()=>showWinScreen(won,k,d), 950);
    }
  }
}
...
window.addEventListener('load',()=>{
  ...
  // Auto-login from saved session (silently, stays on menu)
  ...
  // No session — go straight to menu as guest (skip auth screen)
  authGuest();
  showScreen('menu');
},true); // useCapture=true so this fires BEFORE the existing load handler
 
window.addEventListener('load',()=>{
  cvs=document.getElementById('gc');ctx=cvs.getContext('2d',{alpha:false});
