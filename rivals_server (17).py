...
lobby = {
    ...
    'config': {'mode': 'tdm', 'size': '2v2', 'target': 10},
    'scores': {'red': 0, 'blue': 0},
    'map_seed': 0,
    'host_id': None,
}
    ...
    with lock:
        ...
        if t == 'set_name':
            ...
            if 'size' in cfg and cfg['size'] in SIZE_MAP:
                ...
                size_targets = {'1v1':10,'2v2':10,'3v3':10,'4v4':10}
                if lobby['config']['mode'] == 'tdm':
                    lobby['config']['target'] = size_targets.get(cfg['size'], 10)
            if 'mode' in cfg:
                lobby['config']['mode'] = cfg['mode']
                targets = {'tdm':10,'ffa':8,'koth':20,'ctf':3,'gun':None,'1v1':10}
                lobby['config']['target'] = targets.get(cfg['mode'], 10)
            if 'size' in cfg and cfg['size'] in SIZE_MAP:
                size_targets = {'1v1':10,'2v2':10,'3v3':10,'4v4':10}
                if lobby['config']['mode'] == 'tdm':
                    lobby['config']['target'] = size_targets.get(cfg.get('size', lobby['config']['size']), 10)
            broadcast(lobby_snapshot())
            ...
            if lobby['phase'] != 'playing': return
            # Relay bullet to all other players so they can see it and take damage
            broadcast({'type':'bullet','id':pid,
                       'x':msg.get('x',0),'y':msg.get('y',0),
                       'angle':msg.get('angle',0),
                       'gunId':msg.get('gunId','pistol'),
                       'team':p['team']}, exclude=pid)
            ...
            victim['deaths'] += 1; victim['streak'] = 0
            if lobby['config']['mode'] == 'tdm':
...
const GUNS=[
  {id:'pistol', name:'PISTOL', col:'#94A3B8',dmg:1,rate:.38,spd:640,spread:.06,maxAmmo:12,reload:1.2,pellets:1,range:1.5,tw:2, d:2,f:3,a:3,icon:'🔫'},
  {id:'smg',    name:'SMG',    col:'#22D3EE',dmg:1,rate:.14,spd:560,spread:.16,maxAmmo:25,reload:1.8,pellets:1,range:1.1,tw:1.5,d:1,f:5,a:5,icon:'🔧'},
  {id:'shotgun',name:'SHOTGUN',col:'#F97316',dmg:1,rate:.95,spd:400,spread:.38,maxAmmo:6, reload:2.0,pellets:3,range:.50,tw:2.5,d:3,f:1,a:1,icon:'💥'},
  {id:'sniper', name:'SNIPER', col:'#A3E635',dmg:2,rate:1.5,spd:1200,spread:.01,maxAmmo:4,reload:2.5,pellets:1,range:2.2,tw:3,  d:5,f:1,a:1,icon:'🎯'},
  {id:'assault',name:'ASSAULT',col:'#FB7185',dmg:1,rate:.22,spd:700,spread:.09,maxAmmo:20,reload:1.6,pellets:1,range:1.4,tw:2, d:2,f:4,a:4,icon:'🔴'},
];
...
let currentConfig={mode:'tdm',size:'2v2',target:10};
  ...
  switch(msg.type){
    ...
    case 'state':{
      ...
      if(remotePlayers[msg.id]){
        ...
        const nx=dist>200?msg.x:rp.x+dx*.3;
        const ny=dist>200?msg.y:rp.y+dy*.3;
        if(rp.dead&&!msg.dead)rp._hp=MAX_HP;
        Object.assign(rp,msg);
        rp.x=nx;rp.y=ny; // restore interpolated position after assign
      } else {
    ...
    case 'bullet':{
      // Another player or bot fired — create bullet locally so we see it and can be hit by it
      if(msg.id===myId)break; // our own bullets already created in doShoot
      if(msg.id&&msg.id.startsWith('bot_'))break; // bots create their own bullets locally already
      const shooterRef={team:msg.team, powDamage:false};
      const gun=GUNS.find(g=>g.id===msg.gunId)||GUNS[0];
      bullets.push(new Bullet(msg.x,msg.y,msg.angle,shooterRef,gun,msg.id));
      break;
    }
    case 'kill':
      handleKillMsg(msg);
...
function handleKillMsg(msg){
  ...
  if(msg.scores){gameScores.red=msg.scores.red;gameScores.blue=msg.scores.blue;}
  ...
  if(killerBot){
    ...
    // Don't add to gameScores here — server already sent correct scores in msg.scores above
  }
  ...
  update(dt){
    ...
    if(lp&&!lp.dead&&this.shooterId!==myId&&this.shooterTeam!==lp.team){
      if(Math.hypot(lp.x-this.x,lp.y-this.y)<RAD+5){
        if(lp.shield>0){lp.shield--;lp.hitFlash=.22;this.dead=true;updHpHud();return;}
    ...
    if(this.shooterId===myId&&lp){
      ...
      // Hit remote human enemies — visuals only on shooter's client
      // The victim's client handles actual damage + kill via their incoming bullet
      ...
      if(!target){
        ...
        if(d>90&&d<900&&!this.reloading&&this.ammo>0&&this.botGunCD<=0){
          ...
          if(this.ammo>0&&this.fireCooldown<=0){
            ...
            // Broadcast so all clients see this bot's bullet
            wsSend({type:'bullet',x:this.x,y:this.y,angle:a,gunId:this.gun.id,team:this.team,id:this.id});
          }
...
function doShoot(){
  ...
  for(let i=0;i<p.gun.pellets;i++){
    ...
    bullets.push(new Bullet(p.x,p.y,angle,p,p.gun,myId));
    // Broadcast bullet so other players can see it and take damage from it
    wsSend({type:'bullet',x:p.x,y:p.y,angle,gunId:p.gun.id,team:p.team});
  }
...
function render(){
  ...
  delete remotePlayers[myId];Object.values(remotePlayers).forEach(p=>{if(p&&!p.bot&&p.id!==myId)drawRemotePlayer(p);});
  drawLocalPlayer();
  ctx.restore();
}
...
function updScoreHud(){
  const m=gameConfig||{mode:'tdm',target:10};
  const t=m.target;
